import { LobbyPlayer, ROLE_DISPLAY_NAMES, Side } from '@/types'
...
export function TeamColumn({ side, players, currentUserId, onPlayerClick, selectedPlayerId }: TeamColumnProps) {
  ...
  const getPlayerSlot = (index: number) => {
    ...
    const isClickable = !!onPlayerClick && !isCurrentUser
    ...
    return (
      <div
        ...
        className={`bg-gray-800 rounded-lg p-3 border transition-all ${
          isSelected
            ? 'border-lol-gold ring-2 ring-lol-gold/50'
            : player.isReady
            ? 'border-green-500'
            : isCurrentUser
            ? `border-${sideColor}`
